...
<template>
  ...
  >
    <div
      class="flex flex-row items-center justify-start border-r-[9px] border-r-solid pr-4 pb-4 pt-4"
      :class="{ 'border-r-[red]': (client as IClient).node.status === 'INACTIVE',
        'border-r-[green]': (client as IClient).node.status === 'ACTIVE'
      }"
    >
      <VCardText class="flex-1 pt-0 pb-0 pl-0 pr-1 w-[8%] overflow-hidden">{{
        (client as IClient).node.profile.name || ' '
      }}</VCardText>
      <VCardText class="flex-1 pt-0 pb-0 pl-0 pr-0 w-[8%] overflow-hidden">{{
      ...
      <div class="w-[8%] overflow-hidden pl-8">
        <VCardText
