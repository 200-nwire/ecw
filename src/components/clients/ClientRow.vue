...
<template>
  <VCard v-for="(client, index) in clients" :key="index" class="gap-x-2 flex-1" dir="rtl"
    @click="gotToClientPage((client as IClient).node.id)">
    <div class="flex flex-row items-center justify-between border-r-[9px] border-r-solid pr-4 pb-4 pt-4" :class="{
      'border-r-[red]': (client as IClient).node.status === 'INACTIVE',
      ...
      <div class="hidden lg:block lg:w-[8%] lg:overflow-hidden lg:pl-8">
        <VCardText v-for="(subscription, index) in (client as IClient).node.subscriptions" :key="index"
      ...
      <div class="w-[20%] lg:w-[8%] overflow-hidden">
        <VCardText v-for="(subscription, index) in (client as IClient).node.subscriptions" :key="index"
          ...
          <div class="h-[0.75rem] w-[0.75rem] rounded-[50%]" :class="{
